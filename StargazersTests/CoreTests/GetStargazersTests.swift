...
extension MyURLCommandTests {
    func testGetStargazersRequest() {
        ...
        do {
            try MyServicePerformer(configuration: configure(session))
                .stargazers(for: User(name: "user1", repo: "myrepo"), page: 1) { _ in }
    ...
    func testGetStargazersResponse_withBadData_shouldFail() {
        let session = MockedSession.simulate(failure: MyServiceError.noData) { _ in }
        ...
        do {
            try MyServicePerformer(configuration: configure(session))
                .stargazers(for: User(name: "", repo: ""), page: 0) { result in
                    ...
                    case .failure(let error):
                        guard let mError = error as? MyServiceError else {
                            XCTFail("Unexpected error type!")
                            return
                        }
                        XCTAssert(mError == MyServiceError.noData)
                    }
                }
        } catch { XCTFail("Unexpected error \(error)!") }
    }
