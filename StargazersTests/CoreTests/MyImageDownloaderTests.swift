...
class MyImageDownloaderTests: XCTestCase {
    private var sut: MyImageDownloader?
    ...
    override func setUp() {
        ...
        do {
            mockedImage = UIImage(named: "sample", in: Bundle(for: MyImageDownloaderTests.self), with: .none)
            ...
            let service = MyURLService(session: try XCTUnwrap(session),
                                      dispatcher: SyncDispatcher())
            sut = MyImageDownloader(service: service, cache: try XCTUnwrap(cache))
